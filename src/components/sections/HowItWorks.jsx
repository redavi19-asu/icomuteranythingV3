...
function HowItWorks({ onRequestService }) {
  ...
  const handleOpenChat = () => {
    ...
    if (!existingScript) {
      ...
      script.onload = () => {
        if (window.Tawk_API) window.Tawk_API.onLoaded = () => window.Tawk_API.maximize()
      }
  ...
  return (
    <section id="how-it-works" className="py-24 px-6 relative bg-gradient-to-b from-transparent to-blue-950/10">
      <div className="max-w-6xl mx-auto">
        <motion.div ref={ref} initial={{ opacity: 1, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-20">
        ...
        <motion.div initial={{ opacity: 1, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.45 }} className="mt-20">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300 max-w-3xl mx-auto leading-relaxed text-base md:text-lg">Serving Washington, D.C., Maryland, and Virginia — with remote technology services available far beyond the DMV and travel available when a project needs hands-on support.</p>
          </div>
        ...
        <motion.div initial={{ opacity: 1, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.6 }} className="text-center mt-16">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleOpenChat} className="btn-secondary">Live Chat</motion.button>
          </div>
        </motion.div>
      </div>
    </section>
