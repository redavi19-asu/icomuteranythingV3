...
function SoftwareShowcase() {
  ...
  return (
    <section id="software" className="py-24 px-6 relative">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="relative z-10 grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
            <div>
              <div className="inline-flex px-3 py-1 rounded-full border border-blue-400/30 bg-blue-500/10 text-xs font-semibold text-blue-200 mb-5">Flagship Logistics Software</div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {['Business Workspace', 'Booking', 'Dispatch', 'Driver App', 'Customer Updates', 'Billing'].map((item) => (
                  <span key={item} className="px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm text-gray-200">{item}</span>
                ))}
              </div>
              <div className="mt-9 flex flex-col sm:flex-row gap-3">
                <a href={DISPATCH_URL} target="_blank" rel="noreferrer" className="inline-flex justify-center items-center px-7 py-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold transition-all shadow-lg shadow-blue-900/30">View DispatchOS</a>
                <a href={`${DISPATCH_URL}plans`} target="_blank" rel="noreferrer" className="inline-flex justify-center items-center px-7 py-4 rounded-xl border border-blue-400/30 bg-blue-500/10 hover:bg-blue-500/20 text-blue-100 font-semibold transition-all">Plans & Subscription</a>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[['01', 'Customer requests'], ['02', 'Business dispatches'], ['03', 'Driver completes'], ['04', 'Customer stays updated']].map(([number, label]) => (
        ...
        <div className="text-center max-w-3xl mx-auto mt-24 mb-14">
          ...
          <p className="text-sm md:text-base font-semibold uppercase tracking-[0.18em] text-emerald-300 mb-6">LMS + AMS Business Management Platform</p>
          <p className="section-subtitle">
        ...
        >
          ...
          <div className="relative z-10 grid gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-center">
            <div>
              <div className="inline-flex px-3 py-1 rounded-full border border-emerald-400/30 bg-emerald-500/10 text-xs font-semibold text-emerald-200 mb-5">Unified Learning + Business Operations</div>
              ...
              <div className="mt-9 flex flex-col sm:flex-row gap-3">
                <button type="button" onClick={() => setShowUnifiedPreview(true)} className="inline-flex justify-center items-center px-7 py-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold transition-all shadow-lg shadow-emerald-950/30">View ICA Unified</button>
                <span className="inline-flex justify-center items-center px-7 py-4 rounded-xl border border-emerald-400/20 bg-emerald-500/5 text-emerald-100 font-semibold">Coming Soon</span>
              </div>
            </div>
            <button type="button" onClick={() => setShowUnifiedPreview(true)} className="group block w-full overflow-hidden rounded-2xl border border-white/10 bg-black/35 p-2 md:p-3 shadow-inner text-left" aria-label="Open larger ICA Unified dashboard preview">
              <img src={ICA_UNIFIED_PREVIEW} alt="ICA Unified dashboard preview" className="w-full h-auto rounded-xl border border-white/10 transition-transform duration-300 group-hover:scale-[1.01]" />
      ...
      {showUnifiedPreview && (
        <div className="fixed inset-0 z-[9999] bg-black/90 backdrop-blur-sm p-4 md:p-8 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="ICA Unified dashboard preview" onClick={() => setShowUnifiedPreview(false)}>
          <div className="relative w-full max-w-7xl" onClick={(event) => event.stopPropagation()}>
            <button type="button" onClick={() => setShowUnifiedPreview(false)} className="absolute -top-12 right-0 px-4 py-2 rounded-lg border border-white/20 bg-black/60 text-white hover:bg-white/10">Close</button>
            <img src={ICA_UNIFIED_PREVIEW} alt="Large ICA Unified dashboard preview" className="w-full max-h-[85vh] object-contain rounded-2xl border border-white/10 shadow-2xl" />
